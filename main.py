...
def get_gradient_descent_linear_with_wolfe_condition(c1: float, c2: float):
    ...
    return "Градиентный спуск с одномерным поиском (метод золотого сечения с условием Вольфе)", wrapped
...
def main():
    ...
    gradient_descent_name, gradient_descent = get_gradient_descent_linear_with_wolfe_condition(0.1, 0.5)
    ...
    grad_descent_in_dot = [[call_grad_descent_in_dot(func, grad, gradient_descent, x, y) for x in x_space] for y in y_space]

    print("Количество вызовов функции и ее градиента")
    plt.title(f"Количество вызовов функции и ее градиента")
    func_grad_usage = [[grad_descent_in_dot[iy][ix][2] + grad_descent_in_dot[iy][ix][3] for ix in range(len(x_space))] for iy in range(len(y_space))]
